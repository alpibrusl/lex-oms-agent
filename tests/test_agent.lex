...
fn intg_one_shot_goal_met(db :: conn.ConnDb, log :: trail_log.Log) -> [sql, time, crypto] Result[Unit, Str] {
  let ctx := { db: db, log: log, max_steps: 5 }
...
fn intg_step_limit_enforced(db :: conn.ConnDb, log :: trail_log.Log) -> [sql, time, crypto] Result[Unit, Str] {
  ...
  let ctx := { db: db, log: log, max_steps: 3 }
...
fn intg_submitted_symbols_tracked(db :: conn.ConnDb, log :: trail_log.Log) -> [sql, time, crypto] Result[Unit, Str] {
  let mut_history := []
  let step0 := { step: 0, tool: SubmitOrder({ cl_ord_id: "S1", symbol: "AAPL", side: "buy", quantity: 10 }), outcome: { ok: true, status: 201, body: "{}" }, trail_id: "" }
  let step1 := { step: 1, tool: SubmitOrder({ cl_ord_id: "S2", symbol: "MSFT", side: "buy", quantity: 5 }), outcome: { ok: false, status: 422, body: "{}" }, trail_id: "" }
  let step2 := { step: 2, tool: Observe(Blotter), outcome: { ok: true, status: 200, body: "[]" }, trail_id: "" }
...
fn suite_integration(db :: conn.ConnDb, log :: trail_log.Log) -> [sql, time, crypto] List[Result[Unit, Str]] {
  let __init := srv.init_db(db)
  [intg_one_shot_goal_met(db, log), intg_step_limit_enforced(db, log), intg_submit_order_accepted(db, log), intg_submitted_symbols_tracked(db, log), intg_audit_has_agent_events(db, log)]
}
