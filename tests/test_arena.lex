...
fn test_scenario() -> scenario.Scenario {
  { version: "1", name: "test-ep", seed: 7, episode_start_ms: 1700000000000, tick_ms: 1000, max_steps: 20 }
}
...
fn t_scenario_id_content_addressed() -> Result[Unit, Str] {
  let a := test_scenario()
  let b := { version: "1", name: "test-ep", seed: 8, episode_start_ms: 1700000000000, tick_ms: 1000, max_steps: 20 }
...
fn arena_main() -> [sql, time, crypto, fs_write] Int {
  count_failures([t_verified_roundtrip(), t_tampered_rejected(), t_scenario_id_content_addressed()])
}
