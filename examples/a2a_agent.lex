...
fn nonempty_goal_spec() -> sp.Spec {
  { name: "nonempty_goal",
    quantifiers: [QRecord({ name: "args", fields: [{ name: "text", ty: TStr }] })],
    predicate: EBinop({ op: "!=", lhs: EField({ binding: "args", field: "text" }), rhs: EConst(VStr("")) })
  }
}

fn trade_capability() -> cap.Capability {
  let base := cap.inbound(
    "execute_trade_goal",
    "Execute a natural-language trading goal against the live OMS. Returns outcome summary plus blotter, positions, and risk as artifacts.",
    { title: "TradeGoalArgs", description: "A natural-language trading instruction.", fields: [sch.required_str("text", [StrNonEmpty])] }
  )
...
fn make_handler(db :: conn.ConnDb, log :: trail_log.Log, provider :: prov.Provider, model :: prov.ModelRef)
  ...
  fn (m :: msg.Message) -> [io, time, crypto, random, sql, fs_read, fs_write, net, concurrent, llm, proc] a2a.HandlerOutcome {
    ...
    let artifacts := [
      { name: "blotter",   index: 0, parts: [TextPart(blotter)] },
      { name: "positions", index: 1, parts: [TextPart(positions)] },
      { name: "risk",      index: 2, parts: [TextPart(risk)] },
    ]
...
fn make_agent(db :: conn.ConnDb, log :: trail_log.Log, provider :: prov.Provider, model :: prov.ModelRef) -> a2a.AgentDef {
  let c     := card.make(
    "lex-oms-agent",
    "LLM-backed OMS trading agent — accepts natural-language goals, executes via a typed tool loop, returns blotter/positions/risk artifacts.",
    "0.1.0",
    "http://localhost:4041",
    [trade_capability()]
  )
...
fn app(db :: conn.ConnDb, log :: trail_log.Log, provider :: prov.Provider, model :: prov.ModelRef) -> router.Router {
  let base    := router.new()
  let with_mw := router.use_mw(router.use_mw(router.use_mw(router.use_mw(base,
    mw.body_limit(1048576)),
    mw.request_id()),
    mw.gzip(1024)),
    mw.logger())
...
fn handle(db :: conn.ConnDb, log :: trail_log.Log, provider :: prov.Provider, model :: prov.ModelRef, req :: Request)
    -> [io, time, crypto, random, sql, fs_read, fs_write, net, concurrent, llm, proc] Response {
...
fn main() -> [net, io, time, crypto, random, sql, fs_read, fs_write, concurrent, llm, proc, env] Nil {
  ...
  match conn.connect_sqlite(":memory:") {
    Err(e)  => {
      ...
      net.serve_fn(4041, fn (_r :: Request) -> Response { err_response("db init failed") })
    },
    Ok(db)  => match trail_log.open_memory() {
      Err(m)  => {
        let __e := io.print("trail error: " + m)
        net.serve_fn(4041, fn (_r :: Request) -> Response { err_response("trail init failed") })
      },
      Ok(log) => {
