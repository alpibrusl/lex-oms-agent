...
fn fill_history(db :: conn.ConnDb, history :: List[agent.Step]) -> [sql, time, crypto] Unit {
  let __lex_discard_1 := list.fold(history, 0, fn (idx :: Int, step :: agent.Step) -> [sql, time, crypto] Int {
...
fn run_demo(db :: conn.ConnDb, log :: trail_log.Log, provider :: prov.Provider, model :: prov.ModelRef) -> [sql, time, crypto, net, llm, io] Unit {
  ...
  let trader_goal := str.join(["You are an aggressive momentum trader. Your mandate: concentrate the portfolio into its strongest-performing position. ", "Observe current positions. Identify the symbol with the largest holding and submit a buy order to meaningfully ", "increase it — buy at least 100 shares. Call done once your buy order is accepted by the OMS."], "")
  ...
  let trader_result := match trader_run {
    (res, _hist) => res,
  }
  let trader_hist := match trader_run {
    (_res, hist) => hist,
  }
  ...
  let monitor_goal := str.join(["You are a risk compliance monitor. Policy: no single symbol may exceed 500 shares. ", "First, call observe with target=blotter. Identify any pending BUY orders for symbols that are already ", "at or above the policy limit. Cancel each one with cancel_order to prevent it worsening the breach. ", "Then call observe with target=positions. For every symbol above 500 shares, submit a SELL order to ", "bring it back within limits. Call done when all breaching positions have corrective orders submitted."], "")
