...
fn fmt4(x :: Float) -> Str {
  ...
  let fstr    := if frac < 10 { "000" + int.to_str(frac) }
                 else { if frac < 100 { "00" + int.to_str(frac) }
                 else { if frac < 1000 { "0" + int.to_str(frac) }
                 else { int.to_str(frac) } } }
...
fn run_demo(db :: conn.ConnDb, log :: trail_log.Log, provider :: prov.Provider, model :: prov.ModelRef) -> [sql, time, crypto, net, llm, io] Unit {
  ...
  # ── Phase 2: Black-Scholes pricing (pure Lex, Effects: none) ────────
  let __ := print_section("RISK ENGINE  —  Black-Scholes ATM 30-day put  [pure Lex, no effects]")

  # Inputs as Decimal (lex-risk/src/options.lex accepts Decimal)
  let spot_dec   := d.from_int(nvda_px_int)                      # $875
  let strike_dec := d.from_int(nvda_px_int)                      # ATM strike = spot
  let rate_dec   := { coefficient: 525, exponent: -5 }           # 5.25% annual
  let vol_dec    := { coefficient: 45, exponent: -2 }            # 45% annual vol (NVDA)
  let expiry_days := 30
  let inputs     := bs.option_inputs(spot_dec, strike_dec, rate_dec, vol_dec, expiry_days)

  let put_price_f := bs.put_price(inputs)
  let put_delta_f := bs.put_delta(inputs)   # negative: put moves against spot
  ...
  # Delta-neutral equity hedge: short ⌊|put_delta| × position⌋ shares
  # float.to_int truncates toward zero; put_delta is negative so we negate first
  let delta_abs  := 0.0 - put_delta_f                            # e.g. 0.4321
  let hedge_qty  := float.to_int(delta_abs * int.to_float(position_qty))  # e.g. 432
  let residual   := position_qty - hedge_qty                     # net long after hedge
  ...
  # ── Phase 3: LLM hedging agent ───────────────────────────────────────
  let __ := print_section("LLM HEDGING AGENT  [" + provider.name + " / " + model.model + "]")

  let goal := str.join([
    "You are an autonomous hedging agent. ",
    "The risk engine has computed the following:\n",
    "  Position: long 1,000 NVDA at $875 = $875,000 gross exposure.\n",
    "  Black-Scholes ATM 30-day put: price " + fmt_usd_f(put_price_f) +
      " / share, put delta " + fmt4(put_delta_f) + ".\n",
    ...
    "2. Submit a SELL order: symbol=NVDA, quantity=" + int.to_str(hedge_qty) +
      ", cl_ord_id=HEDGE-NVDA-DELTA.\n",
    "   This is the delta-neutral hedge prescribed by the risk engine.\n",
    "3. Call done with a brief hedge confirmation: position observed, ",
    "delta hedge of " + int.to_str(hedge_qty) + " shares submitted, ",
    "residual net delta +" + int.to_str(residual) + " shares.",
  ], "")
