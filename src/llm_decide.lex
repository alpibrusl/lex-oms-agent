...
fn make_submit_tool() -> lt.Tool {
  let params := {
    title: "SubmitOrderArgs",
    description: "Place a new market order.",
    fields: [
      s.required_str("cl_ord_id", [c.StrNonEmpty]),
      s.required_str("symbol",    [c.StrNonEmpty]),
      s.required_str("side",      [c.StrOneOf(["buy", "sell"])]),
      s.required_int("quantity",  []),
    ],
  }
  lt.define("submit_order", "Submit a market order to the OMS.", params,
    fn (_args :: jv.Json) -> [net, io, proc] Result[jv.Json, e.Errors] { Ok(JObj([])) })
}

fn make_cancel_tool() -> lt.Tool {
  let params := {
    title: "CancelOrderArgs",
    description: "Cancel an existing order.",
    fields: [
      s.required_str("cl_ord_id",      [c.StrNonEmpty]),
      s.required_str("orig_cl_ord_id", [c.StrNonEmpty]),
      s.required_str("symbol",         [c.StrNonEmpty]),
      s.required_str("side",           [c.StrOneOf(["buy", "sell"])]),
    ],
  }
  lt.define("cancel_order", "Cancel an existing OMS order by its cl_ord_id.", params,
    fn (_args :: jv.Json) -> [net, io, proc] Result[jv.Json, e.Errors] { Ok(JObj([])) })
}
...
fn make_observe_tool() -> lt.Tool {
  let params := {
    title: "ObserveArgs",
    description: "Read current OMS state.",
    fields: [
      s.required_str("target", [c.StrOneOf(["blotter", "positions", "risk", "audit"])]),
    ],
  }
  lt.define("observe", "Observe the OMS state: blotter, positions, risk, or audit.", params,
    fn (_args :: jv.Json) -> [net, io, proc] Result[jv.Json, e.Errors] { Ok(JObj([])) })
}
...
fn make_done_tool() -> lt.Tool {
  let params := {
    title: "DoneArgs",
    description: "Signal that the goal is met.",
    fields: [s.required_str("reason", [c.StrNonEmpty])],
  }
  lt.define("done", "Signal that the trading goal is fully achieved.", params,
    fn (_args :: jv.Json) -> [net, io, proc] Result[jv.Json, e.Errors] { Ok(JObj([])) })
}
...
fn collect_deltas(deltas :: List[d.Delta]) -> CollectedResponse {
  list.fold(deltas, { content: "", calls: [], finish_reason: "stop" },
      ...
      match dl {
        ...
        FinishDelta(reason) => {
          let actual := if reason == "stop" {
            if list.is_empty(acc.calls) { "stop" } else { "tool_calls" }
          } else { reason }
...
fn append_chunk(calls :: List[CollectedCall], id :: Str, chunk :: Str) -> List[CollectedCall] {
  list.map(calls, fn (c :: CollectedCall) -> CollectedCall {
    if c.id == id { { id: c.id, name: c.name, args_raw: str.concat(c.args_raw, chunk) } } else { c }
  })
}
...
fn int_field(j :: jv.Json, key :: Str) -> Int {
  match jv.get_field(j, key) { Some(JInt(n)) => n, _ => 0 }
}
